...
def django_main(ssh_host, ssh_port, ssh_login, ssh_password, tunnel_port):
    client = paramiko.SSHClient()
    client.load_system_host_keys()
    client.set_missing_host_key_policy(paramiko.WarningPolicy())

    try:
        client.connect(ssh_host, ssh_port, username=ssh_login, password=ssh_password, key_filename=None,
                       look_for_keys=True)
    except Exception as e:
        print('*** Failed to connect to %s:%d: %r' % (ssh_host, ssh_port, e))
        return

    verbose('Now forwarding remote port %d to %s:%d ...' % (tunnel_port, ssh_host, ssh_port))

    try:
        reverse_forward_tunnel(tunnel_port, 'localhost', 22, client.get_transport())
